...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    score, sow_sad_flag = 0, False
    for i in range(num_rolls):
        current_points = dice()
        score += current_points
        if current_points == 1:
            sow_sad_flag = True
    return score if sow_sad_flag is False else 1
    # END PROBLEM 1
...
def piggy_points(score):
    ...
    min_digit = 0 if squared_score == 0 else 9
    while squared_score > 0:
        current_digit = squared_score%10
...
def more_boar(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    def leftmost_two_digit(score, second_leftmost_digit):
        #This two version of implement will lead to an error when there is a zero in the argument e.g. 104, 10
        #if score <= 9 and second_leftmost_digit:
        #    return score, second_leftmost_digit
        #elif score <= 9 and (second_leftmost_digit is None):
        #    return 0, score
        #if score < 10:
        #    return 0, score if second_leftmost_digit is None else score, second_leftmost_digit
        if score < 10:
            if second_leftmost_digit is None:
                return 0, score
            else:
                return score, second_leftmost_digit
        else:
            return leftmost_two_digit(score//10, score%10)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def commentary_function(score0, score1):
        running_high_value = running_high
        current_increase = score1 - last_score if who else score0 - last_score
        if current_increase > running_high_value:
            running_high_value = current_increase
            print('Player', who, 'has reached a new maximum point gain.', running_high_value, 'point(s)!')
        return announce_highest(who, score1 if who else score0, running_high_value)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    averange = make_averaged(roll_dice, trials_count)
    max_score = 0
    for i in range(1, 10 + 1):
        score = averange(i, dice)
        if score > max_score:
            max_score = score
            dice_number = i
    return dice_number
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    zero_dice_score = piggy_points(opponent_score)
    if more_boar(score + zero_dice_score, opponent_score):
        return 0
    else:
